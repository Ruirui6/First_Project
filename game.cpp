...
//五子棋游戏
//È«ŸÖ±äÁ¿
const char fuhao[11][4] = { "©°", "©Ð", "©Ž", "©À", "©à", "©È", "©ž", "©Ø", "©Œ", "¡ð", "¡ñ" };  //»­ÆåÅÌËùÓÃ¹€Ÿß
int qipan[4][4];
int step = 1;
...
{
	if (qipan[x][y]!=step) cout << "ÇëÑ¡ÔñŒº·œ×ÓœøÐÐÒÆ¶¯£¡" << endl;
	else
	{
		if (mov == 'U')
		{
			if (x == 0) cout << "ÎÞ·šÒÆ¶¯£¡" << endl;
			else
			{
				qipan[x][y] = 0;
				x--;
...
int main()
{
	...
	draw(a);
	cout << "ÇëÊäÈëËùÒÆ¶¯Æå×ÓÓëÒÆ¶¯·œÏò(Æå×ÓÎªÊý×Ö£¬·œÏòÎª¡°U£¬D£¬L£¬R¡±):" << endl;
	while (cin >> x >> y >> mov)
	{
		system("cls");
		move(x, y, mov);
